defmodule Graphene.WS do
  ...
  def start_link(url) do
    :crypto.start
    :ssl.start
    graphene_wss_url = String.to_charlist(url)

    {:ok, sock_pid} = :websocket_client.start_link(graphene_wss_url, __MODULE__, [])
    # websocket_client doesn't pass options to the gen_server, so registering manually
    Process.register(sock_pid, Graphene.WS)
    {:ok, sock_pid}
  end
